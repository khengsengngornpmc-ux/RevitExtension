...
function Resolve-SourceDir {
    ...
    if ($null -eq $latest) {
        throw "No deploy package folder found in .\\release\\CamboBIM_Deploy_Package_*"
    }
...
function Resolve-RuntimeDir {
    ...
    if ([string]::IsNullOrWhiteSpace($runtimePath)) {
        throw "RuntimeDir is empty."
    }

    if (-not (Test-Path -LiteralPath $runtimePath)) {
        throw "RuntimeDir does not exist: $runtimePath"
    }
    ...
    if (-not (Test-Path -LiteralPath $dllPath)) {
        throw "RuntimeDir does not contain " + $assemblyFileName + ": " + $resolved
    }
